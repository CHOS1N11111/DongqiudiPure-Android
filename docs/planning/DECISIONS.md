...
| D-016 | 根 Navigation | 已确定 | `资讯 / 比赛 / 数据 / 我的`（第三次修订） | 已完成 |
...
- 状态：已确定（第三次修订）
- 修订历史：`资讯 / 比赛 / 我的` → `资讯 / 比赛 / 数据` → `资讯 / 比赛 / 数据 / 我的`

compact width 固定使用 `资讯 / 比赛 / 数据 / 我的` 四个 destination；medium/expanded width 使用语义相同的 NavigationRail。搜索与实体详情通过页面入口或 deep link 进入，不增加固定根 tab。
...
复审条件：底栏需要第五项时重新评估信息架构；或 M13 结束后 `我的` 仍然内容稀薄。
