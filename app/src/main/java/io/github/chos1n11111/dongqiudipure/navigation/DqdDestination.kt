 ...
 * 根目的地。
 *
 * 固定五项，不因登录状态增减（DECISIONS.md D-016，第四次修订）。
 *
 * 切分维度不同，所以 [Matches] 与 [Data] 不重复：
 *  - [Matches] 是**日期视角** —— 今天有哪些比赛
 *  - [Data] 是**赛事视角** —— 某个联赛的榜单与赛程
 *
 * 实体详情是内容目的地，从页面入口或 deep link 进入，不占用额外根 tab。
 */
enum class DqdDestination(
    ...
    MainTeam(route = "main-team", labelRes = R.string.nav_main_team, icon = DqdIcons.Add),
